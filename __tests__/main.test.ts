...
describe('When running the action with a created version tag event', () => {
  ...
  const fakeGetChanges = cmd.getChangeLogAssociatedWithTag as jest.MockedFunction<typeof cmd.getChangeLogAssociatedWithTag>
...
describe('When creating a release draft fails', () => {
  ...
  test('it should set the action status to failed with the error message', () => {
    expect(fakeSetFailed).toHaveBeenCalledWith('Failed to create release draft')
  })
